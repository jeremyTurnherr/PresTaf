...
auto = automate(f) // Pour obtenir l'automate de la formule
todot(auto, "res.dot") // Pour exporter l'automate vers le fichier res.dot
// Soient des automates bAuto cAuto
// Permet d'avoir des intersections et unions d'automates.
resAuto = bAuto $\cap$ auto $\cup$ cAuto
\end{lstlisting}
...
\begin{algorithm}
  ...
  \Input{Soit $M = (Q, \Sigma, \delta, q_0, F)$ un automate fini et deterministe}
  \Output{L'automate minimal $ M' = (Q', \Sigma', \delta', q_0', F') $ equivalent à $M$}
\end{algorithm}

>>>>>>> 7ddc772a4c38bccd176ca9295fbd7fe082810a75
